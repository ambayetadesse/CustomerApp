...
let DetailsPage = class DetailsPage {
    ...
    getFood() {
        this.foodService.getAllFood().subscribe(res => {
            ...
            let restaurant = this.listOfRestaurant.find(c => c.accountId == this.id && c.status == "close");
            if (restaurant) {
    ...
    getCategory() {
        this.categoryService.getAllCategory().subscribe(res => {
            let restaurant = this.listOfRestaurant.find(c => c.id == this.resId).category;
            if (restaurant != undefined) {
                for (let i = 0; i < restaurant.length; i++) {
                    let categoryName = res.find(c => c.id == restaurant[i]).categoryName;
                    let data = {
                        id: restaurant[i],
